...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-3 sm:px-6 lg:px-8">
      <div
        className={`mx-auto max-w-7xl transition-all duration-300 rounded-2xl bg-white shadow-[0_4px_20px_rgba(14,31,63,0.08)] border border-navy-100 ${
          scrolled || open || servicesOpen ? 'bg-white/95 backdrop-blur-xl' : 'bg-white'
        }`}
      >
        <div className="container flex h-16 items-center justify-between lg:h-20">
          <a
            ...
            aria-label="K&Sdesk home"
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-navy-900 text-white font-display text-xs font-700 tracking-tighter transition-transform duration-300 group-hover:scale-105 shadow-sm">
              K & S
            </span>
        ...
        {/* Mobile menu */}
        <div
          className={`overflow-hidden bg-white rounded-b-2xl lg:hidden ${open ? 'max-h-[640px] border-t border-navy-100' : 'max-h-0'}`}
